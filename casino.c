	...
	while(1){
		...
		if(strcmp(choix, "poker") == 0){
			...
			void affichageMain(int joueur){
				for (int i = 0; i < 2; ++i)
				{
					...
					if(joueur == -1){
						...
						printf("%s de %s\n", CarteAdversaire[joueur][i].carte, CarteAdversaire[joueur][i].couleur);
					}
				}
			}
			...
			void affichageFinPartie(){
				printf("\n\n");
				int winner = -1, winnerType = -1, winnerCount = 0, winnerVal = 0;
